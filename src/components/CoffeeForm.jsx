...
const CoffeeForm = () => {
  ...
  const calculateTotalPrice = (basePrice, selectedId) => {
    const ingredientSum = ingredients
      .filter((ingred) => selectedId.includes(String(ingred.id)))
      .reduce((sum, ingred) => sum + Number(ingred.price), 0);
    return Number(basePrice || 0) + ingredientSum;
  };
  ...
  const onChange = (e) => {
    ...
    if (id === "ingredients") {
      ...
      const totalPrice = calculateTotalPrice(form.basePrice, values);
    ...
    if (id === "price") {
      const totalPrice = calculateTotalPrice(value, form.ingredients);
